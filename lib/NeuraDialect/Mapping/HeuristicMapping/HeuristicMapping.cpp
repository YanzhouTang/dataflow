...
bool HeuristicMapping::mapWithBacktrack(
    ...
    MappingState &mapping_state) {
  mlir::neura::mapDbg() << "---------------------------------------------------------\n";
  mlir::neura::mapDbg() << "[HeuristicMapping] Starting mapping with "
               << sorted_ops_with_levels.size() << " operations.\n";
  mlir::neura::mapDbg() << "Configuration: MAX Backtrack Depth = "
  ...
  mlir::neura::mapDbg() << "[HeuristicMapping] Filtered "
  ...
  mlir::neura::mapDbg() << "[HeuristicMapping] Materialized operations list:\n";
  for (size_t i = 0; i < materialized_ops.size(); ++i) {
    mlir::neura::mapDbg() << i << " " << *materialized_ops[i].first
  ...
  while (!operation_index_history.empty()) {
    ...
    if (current_op_index >= static_cast<int>(materialized_ops.size())) {
      ...
      mlir::neura::mapDbg() << "[HeuristicMapping] Successfully mapped all "
    ...
    if (max_op_reached - current_op_index > this->max_backtrack_depth) {
      mlir::neura::mapDbg() << "[HeuristicMapping] Max backtrack depth exceeded: "
    ...
    if (candidate_locs.empty()) {
      mlir::neura::mapDbg() << "[HeuristicMapping] No candidate locations found "
      ...
      if (snapshots.empty()) {
        mlir::neura::mapDbg() << "[HeuristicMapping] No more snapshots to restore, "
                     << "mapping failed.\n";
      ...
      mlir::neura::mapDbg() << "[HeuristicMapping] Backtracking to operation "
    ...
    mlir::neura::mapDbg() << "[HeuristicMapping] Found " << candidate_locs.size()
    ...
    if (current_candidate_index >= static_cast<int>(candidate_locs.size())) {
      ...
      mlir::neura::mapDbg() << "[HeuristicMapping] All " << candidate_locs.size()
      ...
      if (operation_index_history.empty()) {
        mlir::neura::mapDbg() << "[HeuristicMapping] FAILURE: No more operations "
    ...
    mlir::neura::mapDbg() << "[HeuristicMapping] Trying candidate "
    ...
    if (placeAndRoute(current_op, candidate_loc, mapping_state)) {
      mlir::neura::mapDbg() << "[HeuristicMapping] Successfully mapped operation "
                   << *current_op << "\n";
      ...
      mlir::neura::mapDbg() << "[HeuristicMapping] Failed to map operation "
