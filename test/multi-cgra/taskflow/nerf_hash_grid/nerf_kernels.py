...
def trilinear_interpolation(pos, pos_grid, level_embeddings,
    ...
    batched_features = torch.ops.neura.gather(
        level_embeddings, batched_indices)
    ...
    for idx in range(8):
        ...
        features = batched_features[idx * N : (idx + 1) * N]
...
def hash_encode(inputs, embeddings, offsets, bound,
    ...
    Args:
        ...
        offsets: Per-level offsets, a sequence of num_levels + 1 ints
            (Python list or int tensor); treated as compile-time constants.
        bound: Scene bounding radius.
    ...
    for level in range(num_levels):
        ...
        # Treats offsets as compile-time constants (Python ints) to keep the
        # slice bounds static and avoid data-dependent symbols during export.
        offset_start = int(offsets[level])
        offset_end   = int(offsets[level + 1])
        hashmap_size = offset_end - offset_start
        level_emb    = embeddings[offset_start:offset_end]
        ...
        # Out-of-bounds check.
        # Expresses the any-reduction as a sum-and-compare so it lowers to
        # linalg (aten.any.dim has no torch-to-linalg conversion).
        out_of_range = ((inputs < 0) | (inputs > 1)).to(torch.int32)
        oob = out_of_range.sum(dim=-1) > 0
    ...
    outputs = torch.cat(level_outputs, dim=-1)
